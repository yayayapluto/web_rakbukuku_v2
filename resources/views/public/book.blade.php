    ...
    <div class="w-[1000px] h-auto mt-8 ml-48">
        <div class="flex">
            ...
            <div class="flex-grow ml-16">
                ...
                <p class="text-sm font-medium text-neutral-500 mb-2">{{ $book->description ?? 'No description available.' }}
                </p>
                ...
                <div class="flex flex-wrap gap-y-4 gap-x-16">
                    ...
                    <div class="">
                        ...
                        </a><!-- Assuming there's a relationship with Category -->
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="fixed bottom-10 left-0 right-0 w-full bg-white ">
        <div class="mx-auto w-[1216px] h-[150px] bg-white drop-shadow-2xl rounded-xl">
            <div class="flex items-center gap-4 h-full p-4">
                ...
                <div class="relative h-24 w-16 shrink-0">
                    @if ($book->cover)
                        <img src="{{ asset($book->cover) }}" alt="{{ $book->title }} cover"
                            class="rounded object-contain h-full w-full" />
                    @endif
                </div>
                    ...
                    <div class="flex gap-2">
                        <span class="text-neutral-500 bg-neutral-50 text-xs rounded py-[2px] px-1 flex items-center gap-1">
                            <svg class="h-3 w-3" fill="currentColor" viewBox="0 0 16 16">
                                <path
                                    d="M11 1H5a2 2 0 0 0-2 2v10c0 1.1.9 2 2 2h7.5a.5.5 0 0 0 0-1H5a1 1 0 0 1-1-1h8.5a.5.5 0 0 0 .5-.5V3a2 2 0 0 0-2-2Z" />
                            </svg>
                            {{ $book->isbn }}
                        </span>
                        <span class="text-neutral-500 bg-neutral-50 text-xs rounded py-[2px] px-1 flex items-center gap-1">
                            <svg class="h-3 w-3" fill="currentColor" viewBox="0 0 16 16">
                                <path
                                    d="M10 10v1h1v-1h-1ZM4.19 1.11l-2.5 2c-.17.13-.19.36-.19.56V5.5c0 .56.19 1.08.5 1.5v7.5c0 .28.22.5.5.5H4V9.5c0-.28.22-.5.5-.5h3c.28 0 .5.22.5.5V15h5.5a.5.5 0 0 0 .5-.5V7c.31-.42.5-.94.5-1.5V3.67c0-.2-.02-.43-.19-.56l-2.5-2A.5.5 0 0 0 11.5 1h-7a.5.5 0 0 0-.31.11Z" />
                            </svg>
                            {{ $book->stock }}
                        </span>
                    </div>
                ...
                @guest
                    <div class="ml-auto">
                        <a class="bg-[#0060AE] hover:bg-[#004681] active:bg-[#003561] text-white py-3 px-4 text-sm font-bold  w-[186px] flex justify-center items-center gap-2 rounded-xl"
                            href="{{ route('login') }}">
                ...
                @auth
                    ...
                    @if (is_null($record) || $record->status == "return")
                        <!-- If no record exists for this book and user, show the borrow button -->
                        <div class="ml-auto">
                            <a class="bg-[#0060AE] hover:bg-[#004681] active:bg-[#003561] text-white py-3 px-4 text-sm font-bold w-[186px] flex justify-center items-center gap-2 rounded-xl"
                                href="{{ route('borrow.form', $book->book_id) }}">
                                <svg fill="currentColor" class="h-5 w-5" viewBox="0 0 20 20">
                                    <path
                                        d="M10.5 2.75a.75.75 0 0 0-1.5 0V9H2.75a.75.75 0 0 0 0 1.5H9v6.25a.75.75 0 0 0 1.5 0V10.5h6.25a.75.75 0 0 0 0-1.5H10.5V2.75Z" />
                                </svg>
                    ...
                    @elseif ($record && $record->status == 'borrow')
                        <!-- If a record exists and the status is 'borrow', show the return button -->
                        <div class="ml-auto">
                            <a class="bg-[#0060AE] hover:bg-[#004681] active:bg-[#003561] text-white py-3 px-4 text-sm font-bold w-[186px] flex justify-center items-center gap-2 rounded-xl"
                                href="{{ route('return.form', $book->book_id) }}">
                                <svg fill="currentColor" class="h-5 w-5" viewBox="0 0 20 20">
                                    <path
                                        d="M10.5 2.75a.75.75 0 0 0-1.5 0V9H2.75a.75.75 0 0 0 0 1.5H9v6.25a.75.75 0 0 0 1.5 0V10.5h6.25a.75.75 0 0 0 0-1.5H10.5V2.75Z" />
                                </svg>
